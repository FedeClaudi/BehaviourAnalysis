...
class BrainViewer:
	...
	def get_structures(self, names=None):
		if names is None: names = self.main_structures
		structures = self.structure_tree.get_structures_by_name(names)
		print(pd.DataFrame(structures))
		self.structures = pd.DataFrame(structures)

		self.structures_details = {s['acronym']:self.structure_tree.get_structures_by_name([s['name']])[0] for s in structures}

		
	def get_structure_wt_experiments(self, structure):
