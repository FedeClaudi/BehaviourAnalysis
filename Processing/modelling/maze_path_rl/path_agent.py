...
class Model:
	...
	def step(self, policy, current, random_action = False):
		...
		if not random_action:
			...
			action_values = softmax(self.Q[policy][current[0]][current[1]])
	...
	def random_walks_f(self, load=True):
		...
		if not os.path.isfile(savename) or not load:
			...
			n_random_steps = round(len(self.shortest_walk) * .5 )
			...
			# add the shortest walk to it
			self.random_walks.append(self.shortest_walk)
